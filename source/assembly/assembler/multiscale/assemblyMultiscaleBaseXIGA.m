function [ M_bb, Mprime_bb, K_bb, Kprime_bb, f_b ] = assemblyMultiscaleBaseXIGA(overlayProblem, baseProblem, time,...
    integrationOrder, overlaySolutionCoefficients, baseSolutionCoefficients,...
    lastConvergedBaseSolution, lastConvergentOverlaySolution, initialTemperature)
%ASSEMBLYMULTISCALEBASEXIGA  assembles the mass, the conductivity matrix
%and load vector of the base XIGA mesh.
%Input:
...
for e=1:baseProblem.N-1
    ...
    for iGP = 1:numberOfIntegrationPoints
        ...
        M_bb(baseProblem.LM(e, 1:ldof_b), baseProblem.LM(e, 1:ldof_b)) = M_bb(baseProblem.LM(e, 1:ldof_b), baseProblem.LM(e, 1:ldof_b)) +...
            ...
            evaluateTemperature(e, rGP(iGP), overlayProblem, baseProblem, lastConvergedBaseSolution))...
...
for e=1:overlayProblem.N
    ...
    for iGP = 1:numberOfComposedIntegrationPoints
        ...
        Mprime_bb(baseProblem.LM(end, 1:ldof_b), baseProblem.LM(end, 1:ldof_b)) = Mprime_bb(baseProblem.LM(end, 1:ldof_b), baseProblem.LM(end, 1:ldof_b)) +...
        ...
        %Diffusion matrix
        K_bb(baseProblem.LM(end, 1:ldof_b), baseProblem.LM(end, 1:ldof_b)) = K_bb(baseProblem.LM(end, 1:ldof_b), baseProblem.LM(end, 1:ldof_b)) +...
            baseProblem.k(globalGPCoord,...
            ...
            * (Bsplines(baseProblem.LM(end,1:ldof_b))' * Bsplines(baseProblem.LM(end,1:ldof_b))) * wGPComposed(iGP) * inverseJacobianX_Xi^2 * baseProblem.F_map(X1,X2);
        
        %Diffusion matrix derivative
        Kprime_bb(baseProblem.LM(end, 1:ldof_b), baseProblem.LM(end, 1:ldof_b)) = Kprime_bb(baseProblem.LM(end, 1:ldof_b), baseProblem.LM(end, 1:ldof_b)) +...
            baseProblem.kDerivative(globalGPCoord,...
            time, evaluateTemperatureComposed(e, rGPComposed(iGP), overlayProblem, baseProblem, overlaySolutionCoefficients, baseSolutionCoefficients))...
            * (Bsplines(baseProblem.LM(end,1:ldof_b))' * Nsplines(baseProblem.LM(end,1:ldof_b))) * (Bsplines * baseSolutionCoefficients * inverseJacobianX_Xi +...
            B * overlaySolutionCoefficients(overlayProblem.LM(e, 1:ldof_o)) * overlayProblem.B_map(X1,X2)) * wGPComposed(iGP) * inverseJacobianX_Xi * baseProblem.F_map(X1,X2);
    end
end
...
numberOfProjectionPoints = length(x);
projectionOperator = zeros(numberOfProjectionPoints, size(overlayProblem.LM, 2) + size(overlayProblem.LME, 2));
projectionOperatorSpline = zeros(numberOfProjectionPoints, size(solutionBaseCoefficients, 1));
integrationDomain = linspace(-1, +1, numel(overlayProblem.reductionOperator));

localCoords = x;
X1 = overlayProblem.coords(e);
X2 = overlayProblem.coords(e + 1);

globalCoords = mapLocalToGlobal(localCoords, X1, X2);
parametricGPCoord = mapGlobalToParametric(globalCoords,...
...
for k=1:length(x)
    ...
    [F, ~] = PODModesAndDerivativesMultiscale(localCoords(k), overlayProblem.modes,...
        overlayProblem.reductionOperator, integrationDomain, e, indexLocalEnrichedNodes);
    ...
    projectionOperatorSpline(k,:) = NIga(:);
end
